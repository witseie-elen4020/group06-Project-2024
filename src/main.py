# This script contains potential parallel code to extract all figures and text from a pdf page AND match figures to text.
# It is based on the Serial version of the same code. 
...
def extract_from_doc(file_path, comm):
    ...
    with fitz.open(file_path) as doc:
        ...
        # Distirbutes pages amoung available processes
        for page_no in range(rank, num_pages, size):
            page = doc[page_no]
    ...
    if rank == 0:
        ...
        while True:
            ...
            if data is None:
                break
            file_path = data
            extract_from_doc(file_path, comm)
